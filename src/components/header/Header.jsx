import { Link, useNavigate } from "react-router-dom";
import { Container, LogoutBtn, Logo } from "../index";
import { useSelector } from "react-redux";

function Header() {
  const authStatus = useSelector((state) => state.auth.status); // auth is name of authslice
  const navigate = useNavigate();

  const navItems = [
    { name: "Home", slug: "/megablog/home", active: true },
    { name: "Login", slug: "/megablog/login", active: !authStatus },
    { name: "Signup", slug: "/megablog/signup", active: !authStatus },
    { name: "Add Posts", slug: "/megablog/add-post", active: authStatus },
    { name: "All Posts", slug: "/megablog/all-posts", active: authStatus },
  ];

  return (
    <header className="py-3 bg-gray-500 shadow ">
      <Container>
        <nav className="flex">
          <div className="mr-4">
            <Link to="">
              {" "}
              <Logo width="70px" />
            </Link>
          </div>

          <ul className="flex ml-auto">
            {navItems.map((item) =>
              item.active ? (
                <li key={item.name}>
                  {" "}
                  <button
                    onClick={() => navigate(item.slug)}
                    className="inline-block px-6 py-2 duration-200 hover:bg-blue-100 rounded-full"
                  >
                    {" "}
                    {item.name}{" "}
                  </button>{" "}
                </li>
              ) : null
            )}
            {authStatus && (
              <li>
                <LogoutBtn />
              </li>
            )}
          </ul>
        </nav>
      </Container>
    </header>
  );
}

export default Header;
